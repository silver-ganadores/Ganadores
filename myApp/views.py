...
def Edit(request, build_id, parts_id):
    
    build = Build.objects.get(id=build_id)
    parts = Parts.objects.get(id=parts_id)
    return render(request, 'form_pc_parts_update.html', {'build':build, 'parts':parts})

def Update(request, build_id, parts_id):
    build = Build.objects.get(id=build_id)
    parts = Parts.objects.get(id=parts_id)
    ...
    if request.method == "POST":
        ...
        parts.processor = request.POST['1model-processesor']
        parts.motherboard = request.POST['motherboard']
        parts.powersupply = request.POST['powersupply']
        parts.ram = request.POST['rams']
        parts.storage = request.POST['size-storage']
        parts.fan = request.POST['fan']
        parts.graphicscard = request.POST['graphics-card']
        parts.chassis = request.POST['pc_case']
        build.save()
        parts.save()
